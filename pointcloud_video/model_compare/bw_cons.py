...
RESEVOIR = 1  # BB
CUSHION = 3  # BB
LOG_FILE  = current_directory+'/results/log_sim_bw_cons'
RESULT_FILE=current_directory+'/results/result_sim_bw_cons'
...
def test(cooked_time,cooked_bw,video_size,fov,dis,time=None):
    ...
    while True:      
        ...
        if fov_count-player_fov_count-player_fractional_time/VIDEO_GOF_LEN>0 and player_fov_count+player_fractional_time>0 and 1==0: #back
            ...
            if back:
                continue
        # 如果没有回退并且当前帧数小于总帧数，则进行新的预测
        if back == 0 and fov_count < FRAME // F_IN_GOF:  # new 
            # 如果当前帧数与播放帧数的差值大于3000，则调整播放时间
            if fov_count - player_fov_count - player_fractional_time / VIDEO_GOF_LEN > 3000:
                # 如果播放时间加上0.1秒大于等于视频帧长度，则增加播放帧数并调整时间戳
            ...
            # 获取新的视频帧延迟和缓冲区
            delay, buffer = net_env.get_video_gof_new(selected_tile, bit_rate)
            # 如果播放时间加上延迟小于当前帧长度，则调整播放帧数和播放时间
