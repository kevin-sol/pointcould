...
for cur_model in range(len(dof_name)):
    ...
    if cur_model>=3:
     ...
     # 输出的大小（假设预测的是多少维数据）
    model = MLPModel(input_size, hidden_size, output_size)  # 重新创建模型实例
    model.load_state_dict(torch.load(current_directory+'/mlp_best_model_'+dof_name[cur_model]+'.pth'))
    model.eval()
...
for cur_model in range(len(dof_name)):
    ...
    #     with open(current_directory+'/output'+str(cur_window)+'.txt', 'w') as file:
    ...
    result_dof[cur_model].to_csv(current_directory+'/mlp_prediction_'+dof_name[cur_model]+'.csv', index=False)