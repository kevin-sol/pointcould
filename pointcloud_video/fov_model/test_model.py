...
for cur_model in range(len(dof_name)):
    ...
    model.load_state_dict(torch.load(current_directory+'/best_model_'+dof_name[cur_model]+'.pth'))
    model.eval()
...
for cur_model in range(len(dof_name)):
    ...
    result_dof[cur_model].to_csv(current_directory+'/prediction_'+dof_name[cur_model]+'.csv', index=False)