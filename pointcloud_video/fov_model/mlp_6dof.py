...
for cur_dof in range(len(dof_name)):
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            if min_loss>test_loss.item():
                ...
                torch.save(model.state_dict(), current_directory+'/mlp_best_model_'+dof_name[cur_dof]+'.pth')
        if epoch % 100 == 0:
    ...
    model.load_state_dict(torch.load(current_directory+'/mlp_best_model_'+dof_name[cur_dof]+'.pth'))
    model.eval()
    model = model.to(device)
    with torch.no_grad():
        test_outputs = model(X_test)
